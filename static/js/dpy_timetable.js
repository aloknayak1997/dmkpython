    ...
    $("#excel_table").droppable({
        ...
        drop: function(event, ui) {
            ...
            if (row >= 2 && col >= 1) {//Excluding 1st,2nd row and 1st column
              ...
              var json = {'day_id':day_id,'start_time':st,'end_time':et,'lecture_no':lec_no,'css':sid,'user':tid};

              $.ajax({
                  url: timetable_urls.managett,
                  method: "POST",
                  data:{ 'data':JSON.stringify(json),'ic_id':ic_id,csrfmiddlewaretoken: g_csrftoken}
              }).done(function(resp) {
                  if(resp.status == true){
