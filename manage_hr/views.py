...
def insert_emp(request):
    ...
    u = DpyUsers.objects.all().filter(first_name=data[0],middle_name=data[1],username=data[0],last_name=data[2],mobile=data[3],email=data[4],gender=data[8])
    if u.count() >= 1:
...
def addleave(request):
    ...
    u = models.Leave.objects.all().filter(emp_id=data[0],institute_id=inst_id,emp_name=data[1],leave_type=data[2],start_date=data[3],end_date=data[4])
    if u.count() >= 1:
            data = {'Status': False, 'Message': "Leave Already exists"}
    else:
