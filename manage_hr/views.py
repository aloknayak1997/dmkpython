...
def salary(request):
    ...
    return render(request ,'manage_hr/generate_salary_slip.html',{'data':user,'range': range(1980,2021),'months':months, 'institute': institute})
...
def insert_emp(request):
    ...
    print(inst_id)
    data = request.POST.getlist('studentData[]')
    u = models.DpyEmployee(institute_id=inst_id,first_name=data[0],middle_name=data[1],last_name=data[2],mobile_number=data[3],email=data[4],date_of_join=data[5],designation=data[6],department=data[7],gender=data[8],basic_salary=0,casual_leaves=20,paid_leaves=20,sick_leaves=20)
    u.save()
    empid=u.id
...
def structure(request):
    ...
    try:
        ...
        v = models.DpyEmployee.objects.all().filter(id=data[0]).values('first_name','last_name','basic_salary','designation');
        name = v[0]['first_name']+" "+v[0]['last_name']
        bs =v[0]['basic_salary']
        desg =v[0]['designation']
        hra =u[0]['hra']
        ta =u[0]['ta']
        da =u[0]['da']
        med =u[0]['med']
        ...
        pt = u[0]['pt']
        pf = u[0]['pf']
        data ={'Status':True,'Message':'Successfull','name':name,'bs':bs,'hra':hra,'ta':ta,'da':da,'med':med,'bonus':bonus,'spal':spal,'others':others,'tds':tds,'pt':pt,'pf':pf,'desg':desg}
    except IndexError:
