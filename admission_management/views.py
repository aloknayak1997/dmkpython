...
from admission_management.serializers import InstituteAdmissionProspectSerializer,DpyInstituteAdmissionsUsers
import json
...
class Admission(APIView):
    ...
    def get(self, request, format=None):
        ...
        return render(request,"admission_management/admission.html",{'total_classes':total_classes,'institute_id':institute_id,'range':range(2010,2019)})
    ...
    def post(self,request, format=None):
        ...
        if userSerial.is_valid():
            ...
            last_exam = json.loads(request.data.get('last_exam'))
            print(last_exam)
            # user.form_content=last_exam.replace()
            # print(user.form_content)

            str = "{ \"college_name\" :"
            str += "\""+last_exam['college_name']+"\""

            str += ", \"board\" :"
            str += "\""+last_exam['board']+"\""
            
            str += ", \"seat_no\" :"
            str += "\""+last_exam['seat_no']+"\""
            
            str += ", \"year\" :"
            str += "\""+last_exam['year']+"\""
            
            str += ", \"marks_scored\" :"
            str += "\""+last_exam['marks_scored']+"\""
            
            str += ", \"out_of_marks\" :"
            str += "\""+last_exam['out_of_marks']+"\""
            
            str += ", \"percentage\" :"
            str += "\""+last_exam['percentage']+"\""
            
            str += ", \"grade\" :"
            str += "\""+last_exam['grade']+"\""
            str += "}"
            
            user.form_content = str
...
def view_enquiry(request):
    if request.method == 'GET':
         queryset = DpyInstituteAdmissionProspect.objects.filter(institute_id = request.session['institute_id'])
...
class View_admission(APIView):
    
    def get(self,request,format=None):
        admission_enquiries = DpyInstituteAdmissionsUsers.objects.all().filter(institute_id=request.session['institute_id'])
        total_classes = DpyInstituteClass.objects.all().filter(institute_id = request.session['institute_id'])
        return render(request,"admission_management/view_admission.html",{'admission_enquiries':admission_enquiries,'total_classes':total_classes})
